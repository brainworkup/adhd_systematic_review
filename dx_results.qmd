...
demographics_table_neuroimaging <- demographics_data_neuroimaging |>
  gt() |>
  tab_header(
...
I understand you're looking for a more comprehensive table of neuropsychological tests.
After thoroughly reviewing the provided document, I've compiled all sensitivity and specificity data that's available.
The document you shared is actually a research protocol for a systematic review rather than the completed review itself, so it contains relatively limited specific sensitivity/specificity values.
...
tbl_neuro <- create_demographics_table(
  data = df_neuro,
